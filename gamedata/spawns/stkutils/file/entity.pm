...
use constant properties_info => (
	...
	{ name => 'm_speed',	type => 'f32',	default => 0.0 },
	{ name => 'm_amount',	type => 'f32',	default => 0.0 },
	{ name => 'm_smap_jitter',	type => 'f32',	default => 0.0 },
);
sub init {
	cse_alife_dynamic_object_visual::init(@_);
	cse_ph_skeleton::init(@_);
...
sub state_read {
	...
	if ($_[0]->{version} == 118) {
		$_[1]->unpack_properties($_[0], (properties_info)[27..29]);
	}
}
sub state_write {
	if ($_[0]->{version} > 20) {
	...
	if ($_[0]->{version} == 118) {
		$_[1]->pack_properties($_[0], (properties_info)[27..29]);
	}
}
sub state_import {
	if ($_[0]->{version} > 20) {
	...
	if ($_[0]->{version} == 118) {
		$_[1]->import_properties($_[2], $_[0], (properties_info)[27..29]);
	}
}
sub state_export {
	if ($_[0]->{version} > 20) {
	...
	if ($_[0]->{version} == 118) {
		$_[1]->export_properties(undef, $_[0], (properties_info)[27..29]);
	}
}
#######################################################################
